...
Item {
    ...
    function punching() {
        if (typeof (_hostile) !== 'undefined') {
            ...
            case false:
                //punch right
                for (indexId = 0; indexId < _hostile.model; indexId++) {
                    if (((((_hostile.itemAt(
    ...
    Behavior on x {
        ParallelAnimation {
            ...
            //                ScriptAction {
            //                    script: {
            //                        mainHero.state !== "die" ? [blinking.running = true, state = Qt.binding(
            //                                                        function () {
    ...
    Behavior on y {
        ParallelAnimation {
            ...
            //                ScriptAction {
            //                    script: {
            //                        mainHero.state !== "die" ? [walking.running = true] : {}
            //                    }
            //                }
            //                PauseAnimation {
            //                    duration: 40
        ...
        Transition {
            ...
            ParallelAnimation {
                ...
                SequentialAnimation {
                    ...
                    ScriptAction {
                        script: mainHero.defense = mainHero.defenseDef
                    }
                }
            }
        }
                        ...
                        if (event.key === 74 || event.key === 1054) {
                            if (health > 0) {
                                switch (level.state) {
                                    ...
                                    break
                                    default:
                                    latestActivity == "attack1" ? [punching2.running = true, latestActivity = "attack2"] : [punching1.running = true, latestActivity = "attack1"]
                                    break
                                }
                            }
                            event.accepted = true
                                ...
                                switch (level.state) {
                                    ...
                                    if (semenSideRot == 0) {
                                        ...
                                        if (mainHero.x <= moveH) {
                                            ...
                                            if (room.width > pageLoader.width
                                                && room.width
                                                - (mainHero.hitbox.x + mainHero.hitbox.width
                                                   + way.x) >= pageLoader.width / 2) {
                                                if (mainHero.hitbox.x
                                                    - mainHero.hitbox.width - way.x <= 0)
                                                level.x = 0
                                                else
                                                level.x = -mainHero.hitbox.x
                                                + mainHero.hitbox.width + way.x
                                            }
                                            x -= moveH
                                        }
                                    }
                            ...
                            if (health > 0) {
                                switch (level.state) {
                                    ...
                                    if (semenSideRot == -180) {
                                        ...
                                        if (mainHero.x >= (way.width - hitbox.width - moveH)) {
                                            x = way.width - hitbox.width
                                        } else {
                                            if (room.width > pageLoader.width
                                                && mainHero.x + way.x >= pageLoader.width / 2) {
                                                if (mainHero.x - way.x >= room.width
                                                    - pageLoader.width) {
                                                    level.x = -room.width + pageLoader.width
                                                } else
                                                level.x = -mainHero.x + way.x
                                            }
                                            x += moveH
                                        }
                                    }
                            ...
                            if (health > 0) {
                                switch (level.state) {
                                    ...
                                    break
                                    default:
                                    if (mainHero.y >= -hitbox.height + way.height - moveV) {
                            ...
                            if (health > 0) {
                                switch (level.state) {
                                    ...
                                    break
                                    default:
                                    if (mainHero.y <= -hitbox.height + moveV) {
                                        y = -hitbox.height
                                    } else {
                                        y -= moveV
                                    }
                            ...
                            if (health > 0) {
                                switch (level.state) {
                                    ...
                                    break
                                    default:
                                    inventory.item.opacity = inventory.item.opacity
                                    > 0 ? 0 : 1 // !inventory.item.visible
                            ...
                            switch (level.state) {
                                case "stopGame":
                                subMenu.item ? [subMenu.item.opacity = 0, subMenu.item.enabled
                                                = false] : console.log(
                                ...
                                > 0 ? [inventory.item.opacity = 0, inventory.item.enabled
                                       = false] : subMenu.item ? [subMenu.item.opacity = 1, subMenu.item.enabled = true] : console.log(
                                                                     "SubMenu not loaded yet")
                                break
                            }
